...
import { MAX_INT64, currentTimestamp, getRawAmount, isMobilePlatform, isNullOrEmpty, parseAmountWithDecimals } from "../../utils/common.utils";
...
export default function TokenSend({ balance, keys, ticker, decimals, tokenEntity, tokenBalance, isMobile }: SendProps) {
  ...
  const handleSend = async () => {
    try {
      ...
      let tx = await buildAndSignTransferTransaction(keys, toAddress, amt.toString(), undefined, false, tokenEntity?.token);
      setFinalTx(tx);
      ...
      if (decMn) {
        try {
          ...
          let t: TransactionEntity = {
            ...
            state: 'outgoing',
            tokenAmount: getRawAmount(amount, decimals),
        ...
        <Modal.Body>
          <Table>
            <tbody>
              ...
              <tr>
                <td>Amount:</td>
                <td>{new bigDecimal(amount).getPrettyValue()} {ticker}</td>
              </tr>
              <tr>
                <td>Fee:</td>
